...
class IDSSignature(Base):
    """
    Firma conductual del IDS. Detecta COMPORTAMIENTO (cuenta eventos por ventana
    de tiempo), no contenido de payload. El motor lee estas filas y según
    `tipo_firma` aplica la lógica de detección correspondiente.
    """
    ...
    track_by         = Column(String(8), default="src")     # src | dst
    puerto           = Column(Integer, nullable=True)       # puerto específico, o NULL = cualquiera
...
# Catálogo de 15 firmas conductuales precargadas (basadas en umbrales Snort).
SIGNATURES_SEED = [
...
# Tipos de firma válidos (para validar al crear/editar desde el frontend)
TIPOS_FIRMA = (
    "port_scan", "null_scan", "fin_scan", "xmas_scan", "udp_scan", "ping_sweep",
    "syn_flood", "icmp_flood", "udp_flood", "ping_of_death",
    "brute_force", "land_attack", "conn_flood",
)
